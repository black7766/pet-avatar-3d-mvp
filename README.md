...
- 抠图在透明过渡带做前景色恢复，并对主体轮廓附近的低饱和黄绿色反光做选择性去溢色；高饱和绿色虹膜和饰品不处理。
- 锁定首尾帧时保留完整端点，不再执行旧的头尾裁剪和交叉淡化。
- 同源 A/B：视频 tokens `653,400 → 523,800`，增加最终轮廓光晕修复后本地抠图墙钟 `75.7s → 46.5s`，平均循环缝 `0.991% → 0.290%`，单宠刊例成本约 `¥5.73 → ¥5.69`。
