        ...
        for flow in ("before", "after"):
            ...
            cards.append(f"""
              <article class="demo-card" data-style="{style}" data-flow="{flow}">
                <div class="card-head"><div><strong>{FLOW_LABELS[flow]}</strong><span>{'同一坐姿驱动全部动作' if flow == 'before' else '目标状态首尾帧 + 内存并行抠图'}</span></div><b data-role="frame-count">{first['frames']} 帧</b></div>
                <div class="stage"><img draggable="false" src="/{pet_id}/anim_idle.webp?v=ab5" alt="{STYLE_LABELS[style]}{FLOW_LABELS[flow]}" /></div>
...
document.querySelectorAll('button[data-clip]').forEach(button=>button.addEventListener('click',()=>{{
 ...
 document.querySelectorAll('.demo-card[data-style][data-flow]').forEach(card=>{{
  ...
  card.querySelector('img').src=`${{asset.web_path}}?v=ab5_${{clip}}`; card.querySelector('[data-role="frame-count"]').textContent=`${{asset.frames}} 帧`; const spans=card.querySelectorAll('.live-metrics span');
